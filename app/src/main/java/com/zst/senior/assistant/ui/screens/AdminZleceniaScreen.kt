...
private fun SharedZleceniaListScreen(
    ...
    val actionState by viewModel.actionUiState.collectAsState()

    var zlecenieToDeleteId by remember { mutableStateOf<String?>(null) }
    ...
    // Obsługa komunikatów po akcji (np. usunięciu)
    LaunchedEffect(actionState) {
        if (actionState is ZlecenieActionUiState.Success) {
            snackbarHostState.showSnackbar((actionState as ZlecenieActionUiState.Success).message)
            viewModel.resetActionState()
        } else if (actionState is ZlecenieActionUiState.Error) {
            snackbarHostState.showSnackbar((actionState as ZlecenieActionUiState.Error).message)
            viewModel.resetActionState()
        }
    }

    if (zlecenieToDeleteId != null) {
        AlertDialog(
            onDismissRequest = { zlecenieToDeleteId = null },
            icon = { Icon(Icons.Default.Delete, null, tint = MaterialTheme.colorScheme.error) },
            title = { Text(stringResource(R.string.admin_orders_delete_confirm_title)) },
            text = { Text(stringResource(R.string.admin_orders_delete_confirm_message)) },
            confirmButton = {
                Button(
                    colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error),
                    onClick = {
                        viewModel.usunZlecenie(zlecenieToDeleteId!!, isArchive)
                        zlecenieToDeleteId = null
                    }
                ) {
                    Text(stringResource(R.string.admin_orders_delete_btn), color = Color.White)
                }
            },
            dismissButton = {
                TextButton(onClick = { zlecenieToDeleteId = null }) {
                        ...
                        LazyColumn(
                            ...
                            items(zlecenia) { zlecenie ->
                                ZlecenieItemPublic(
                                    ...
                                    onDeleteClick = {
                                        zlecenieToDeleteId = zlecenie.id
                                    }
                                )
                            }
                        }
